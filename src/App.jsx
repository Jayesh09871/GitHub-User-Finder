...
function UserCard({ user, onClick }) {
  return (
    <div className="flex items-center bg-white shadow rounded p-3 mb-2 hover:bg-blue-50 cursor-pointer" onClick={onClick}>
      <img src={user.avatar_url} alt={user.login} className="w-12 h-12 rounded-full mr-4" />
      <div className="flex-1">
        <div className="font-bold">{user.login}</div>
      </div>
      <button className="ml-4 px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">View</button>
    </div>
  );
}
...
function UserList({ users, onSelect }) {
  if (!users.length) return <div className="text-center text-gray-500 mt-4">No users found.</div>;
  return (
    <div className="mt-4 space-y-2">
      {users.map(user => (
        <UserCard key={user.id} user={user} onClick={() => onSelect(user.login)} />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center px-2 py-6">
      <div className="w-full max-w-xl">
        <h1 className="text-3xl font-bold text-center mb-6 text-blue-600">GitHub User Finder</h1>
